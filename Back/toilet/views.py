...
def search_toilets(request):
    try:
        ...
        # 주소, 이름 등 여러 필드에서 검색
        toilets = Toilet.objects.filter(
            Q(name__icontains=keyword) |  # 화장실 이름
            Q(address__icontains=keyword)  # 주소
        )
        ...
        for toilet in toilets:
            toilet_data = {
                ...
                'total_stalls': {
                    'male': toilet.male_stalls + toilet.male_urinals,
                    'female': toilet.female_stalls,
                },
                'is_accessible': toilet.is_accessible,
                'management': {
                    'agency': toilet.managing_agency_name or "정보 없음",
                    'phone': toilet.managing_agency_phone or "정보 없음"
                },
                'security': {
                    'cctv': toilet.cctv_installed,
                    'emergency_bell': toilet.emergency_bell_installed
                },
                'facilities': {
                    'diaper_table': toilet.diaper_change_table_location or "없음",
                    'opening_hours': toilet.opening_hours if toilet.opening_hours else "정보 없음",
                    'opening_hours_detail': toilet.opening_hours_detail if toilet.opening_hours_detail else "상세 정보 없음"
                },
                'latitude': toilet.latitude,
                'longitude': toilet.longitude
            }
            results.append(toilet_data)
